...
public interface AlimentacionPacienteRepository extends JpaRepository<AlimentacionPaciente, Integer> {
    ...
    @Modifying
    @Query(value = "update alimentacion_paciente set alimentacion=:alimentacion, jornada=:jornada, fecha_hora=:fecha_hora, cantidad=:cantidad where id_alimentacion_paciente=:id_alimentacion_paciente", nativeQuery = true)
    public void actualizarAlimentacionPaciente(@Param("id_alimentacion_paciente")int id_alimentacion_paciente,
                                               @Param("alimentacion")int alimentacion,
                                               @Param("jornada")int jornada,
                                               @Param("fecha_hora")Date fecha_hora,
                                               @Param("cantidad")int cantidad);
    @Query(value = "select * from alimentacion_paciente ap join alimentacion a on a.id_alimento=ap.alimentacion join paciente p on p.cedula=ap.paciente where ap.paciente=:cedula", nativeQuery = true)
    public List<AlimentacionPaciente> findAllByPaciente(@Param("cedula") long cedula);

    @Query(value = "select * from alimentacion_paciente ap join alimentacion a on ap.alimentacion=a.id_alimento join paciente p on p.cedula=ap.paciente where ap.paciente=:cedula and ap.fecha_hora between :fecha1 and :fecha2", nativeQuery = true)
